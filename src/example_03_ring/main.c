 ...
 * predecessor using non-blocking communication (MPI_Isend/MPI_Irecv).
 *
 * Build example (manual):
 * @code
 * mpicc -std=c11 -Wall -Wextra -Wpedantic -O3 -march=native -flto main.c -o example_03_ring
 * @endcode
 *
 * @note The CMake build system places the binary in `build/bin/example_03_ring`.
 *
 * Run example (manual):
 * @code
 * mpirun -n 4 ./example_03_ring
 * @endcode
 *
 * Expected output:
 * @code
...
/**
 * @brief Performs a ring exchange of data.
 *
 ...
 * @param[in] next Rank of the successor.
 * @param[in] prev Rank of the predecessor.
 */
static void ring_exchange(const void *sendbuf,
                          void *recvbuf,
                          const int count,
                          const int next,
                          const int prev)
...
int main(int argc, char *argv[])
{
         ...
         msg_size *= 2)
    {
        ...
        {
            ring_exchange(sendbuf, recvbuf, count, next, prev);
        }
        ...
        const double exchange_us = ring_time * 1e6;

        const double bw_MiBs =
            ((double)msg_size * size) /
            ring_time /
            (1024.0 * 1024.0);

        const uint64_t csum =
            checksum(recvbuf, msg_size);

        MPI_CHECK(MPI_Gather(&bw_MiBs,
                             1,
                             MPI_DOUBLE,
                             all_bw,
                             1,
        ...
        if (rank == 0)
        {
            ...
            for (int r = 0; r < size; r++)
            {
                const int expected_sender =
                    (r - 1 + size) % size;

                const uint64_t expected =
                    (uint64_t)expected_sender * (uint64_t)msg_size;
            ...
            if (msg_size == MAX_MSG_SIZE)
            {
                ...
                for (int r = 0; r < size; r++)
                {
                    ...
                    const uint64_t expected_checksum =
                        (uint64_t)prev_rank * (uint64_t)msg_size;
